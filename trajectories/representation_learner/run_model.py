...
def train_meta_model(
    ...
    start_epoch = (epoch + 1) if reloaded else 0
    if tqdm is not None:
        epoch_rng = tqdm(
        ...
        epoch_rng = range(start_epoch, args.epochs)
    ...
    for epoch in epoch_rng:
        ...
        # track train averages this epoch
        ...
        for i, batch in enumerate(train_bar):
            ...
            train_count += 1
            train_bar.set_postfix(loss=train_loss_sum / max(1, train_count))
            ...
            # VICReg logging (TRAIN) — safe access
            if wandb_state == 'pretrain' and args.do_vicreg and isinstance(all_outputs, dict):
                for key in ['sim_loss', 'cov_loss', 'var_loss']:
                    for prefix in ['traj', 'elem']:
                        full_key = f'{prefix}_{key}'
                        if full_key in all_outputs:
                            _ = all_outputs[full_key]  # or log it if you re-enable wandb
        ...
        epoch_train_loss = train_loss_sum / max(1, train_count)
        ...
        if tqdm is None: 
            ...
            epoch_rng.set_description(f"Epoch {epoch+1}/{args.epochs}")
            epoch_rng.set_postfix(train_loss=epoch_train_loss)
        ...
        if tqdm is not None:
            ...
            val_bar = tuning_dataloader

        # track val averages this epoch
        ...
        for i, batch in enumerate(val_bar):
            ...
            # VICReg logging (VAL) — safe access
            if wandb_state == 'pretrain' and args.do_vicreg and isinstance(all_outputs, dict):
                for key in ['sim_loss', 'cov_loss', 'var_loss']:
                    for prefix in ['traj', 'elem']:
                        full_key = f'{prefix}_{key}'
            ...
            val_loss_sum += float(total_loss.detach())
            val_count += 1
            val_bar.set_postfix(loss=val_loss_sum / max(1, val_count))

        epoch_val_loss = val_loss_sum / max(1, val_count)
        tqdm.write(f"Epoch {epoch+1}/{args.epochs}: "
                f"train_loss={epoch_train_loss:.4f}, "
                f"val_loss={epoch_val_loss:.4f}")
        epoch_rng.set_postfix(train_loss=epoch_train_loss, val_loss=epoch_val_loss)
        ...
        if cond:
            ...
            if tqdm is None: 
                ...
                epoch_rng.set_description(f"Epoch {epoch+1}/{args.epochs}")
                epoch_rng.set_postfix(loss=float(total_loss.detach()))
...
def load_datasets(args, task='ssl'):
    ...
    # map to MC-MED splits
    split_map = {'train': 'train', 'tuning': 'val', 'test': 'test'}
